...
def load_real_data(data_dir, normal_pattern='*normal*.csv', fault_pattern='*fault*.csv',
                   signal_column='signal', train_ratio=0.8, segment_length=1024, overlap=0.5):
    """
    ...
    # if not normal_files:
    #     print("错误: 没有找到正常数据文件，无法训练模型")
    #     return load_example_data()  # 如果没有找到文件，返回示例数据
...
def main():
    ...
    if args.mode == 'test' or args.mode == 'demo':
        ...
        if 1 in test_labels:
            ...
            # 为了避免后续错误，提供默认值
            fault_result = {'Fault_Detected': False, 'T2': 0, 'T2_Threshold': 0, 'SPE': 0, 'SPE_Threshold': 0,
                            'Fault_Score': 0}
            fault_plot_result = fault_result.copy()
...
#         sys.argv[0],  # 保留原始脚本名称
#         '--mode', 'train',  # 训练模式
#         '--data_path', r'E:\Github\machinery_fault_diagnosis\train_data',  # 训练数据目录
#         '--save_path', './models',  # 模型保存目录
#         '--visualize'  # 启用可视化
#     ]
#
#     # 将配置参数添加到PATH_CONFIG和DATA_CONFIG
#     # 注意：这种方式会修改配置，确保在导入配置之后执行
#     DATA_CONFIG.update({
#         'normal_pattern': 'normal*.csv',  # 正常数据文件名模式
#         'fault_pattern': 'fault*.csv',  # 故障数据文件名模式
#         'sampling_rate': 10000,  # 采样率(Hz)
#         'segment_length': 2048,  # 信号分段长度
#         'segment_overlap': 0.5  # 分段重叠比例
#     })
#
#     PATH_CONFIG.update({
#         'model_dir': './models',  # 模型目录
#         'model_path': './models/fault_diagnosis_model.pkl'  # 完整模型路径
#     })
#
#     # 确保模型目录存在
...
if __name__ == '__main__':
    ...
    model_path = './models/fault_diagnosis_model.pkl'
